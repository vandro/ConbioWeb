        ...
        return [
            'access' => [
                ...
                'rules' => [
                    ...
                    [
                        'actions' => ['account', 'view-profile', 'profile', 'resend-change', 'cancel'],
                        'allow' => true,
                        'roles' => ['@'],
                    ],
                    [
...
$this->layout = '//main-login';
        ...
        if ($model->load($post) && $model->validate()) {
            ...
            if (strpos(yii::getAlias('@web'), 'backend') !== false && $user->role->id!==$role::ROLE_USER) {
                $returnUrl = $this->performLogin($model->getUser(), $model->rememberMe);
                return $this->redirect($returnUrl);
            }else{
                if (strpos(yii::getAlias('@web'), 'frontend') !== false) {
                    $returnUrl = $this->performLogin($model->getUser(), $model->rememberMe);
                    return $this->redirect($returnUrl);
                }
        ...
        if (strpos(yii::getAlias('@web'), 'backend') !== false) {
                return $this->render('backendLogin', compact("model"));
        }else{
                return $this->render('frontendLogin', compact("model"));
        }
    }
...
//            if ($newPath=$profile->upload()) {
//                // file is uploaded successfully
//                $profile->photo =$newPath;
//            }
//            $profile->save(false);
...
//        }
        ...
        return $this->render("viewProfile", compact("profile"));
    }
    /**
     * Profile
     */
